<?php
	// TEST SI L'USER A APPUYER SUR ENVOYER (CREATION MAT)
	if(isset($_POST['ajouter']) && $_POST['ajouter']=="Ajouter")
	{
		$countex=mysql_query('SELECT COUNT(*) a FROM matiere WHERE nom_mat=\''.$_POST['nom_mat'].'\' AND id_promo=\''.$_POST['prom'].'\'');
		// Test si la departement existe deja
		if(!mysql_result($countex,0))
		{
			// Requete d'ajout
			mysql_query('INSERT INTO matiere (nom_mat,id_promo) VALUES ("'.$_POST['nom_mat'].'","'.$_POST['prom'].'")');
		}
		else
		{
	...
	if(isset($_GET['suppr']))
	{
		...
		mysql_query('DELETE FROM matiere WHERE id_mat="'.$_GET['suppr'].'"');
	}
	
	// TEST SI L'USER A VALIDER LES MODIFICATIONS (MAT)
	if(isset($_POST['validemodif']) && $_POST['validemodif']=="Valider")
	{
		mysql_query('UPDATE matiere SET nom_mat="'.$_POST['n_nom_mat'].'", id_promo="'.$_POST['n_promo_mat'].'" WHERE id_mat="'.$_POST['id_mat'].'"');
	}
	...
	<!-- BLOC CREATION MATIERE -->
		<div id="bloccreamat" style="display:none">
			<form action="index.php?p=gest_mat" method="post">
				...
				<?php
					$promo=mysql_query('SELECT * FROM promotion, departement WHERE promotion.id_dpt=departement.id_dpt');
					while($array=mysql_fetch_array($promo))
					{
						echo '<option value='.$array[id_promo].'>'.$array[nom_dpt].' '.$array[nom_promo].' '.$array[annee].'</option>';
					}
				?>
				</select><br>
...
<!-- ######### AFFICHAGE TITRE TABLEAU ######### -->
			<?php	
			$countmat=mysql_query('SELECT COUNT(*) a FROM matiere');
			?>
			<div id="bloctitre"><div class="nomtitre">Mati&egrave;re</div><div class="nomtitre">Promotion</div></div>	
	
<!-- ######### AFFICHAGE CONTENU TABLEAU ######## -->
			<form action="index.php?p=gest_mat" method="post">
				<?php
					$query1=mysql_query('SELECT * FROM matiere, promotion, departement WHERE matiere.id_promo=promotion.id_promo AND promotion.id_dpt=departement.id_dpt ORDER BY nom_dpt, nom_mat');
				...
				<div class="contenutab">
					...
					<div id="<?php echo 'B'.$tab1[id_mat] ?>" class="nomtitre"><?php echo $tab1[nom_dpt].' '.$tab1[nom_promo].' '.$tab1[annee]; ?></div>
					<div onclick="modif_mat(<?php echo $tab1[id_mat]; ?>, '<?php echo $tab1[nom_mat]; ?>');" class="blocmodif"><img class="imgmodif" src="images/set.png"></div>
...
<script>
	...
	function modif_mat(id,nom)
	{
		...
		blocB.innerHTML="<div id='listeDer'><select id='listeDer2' name='n_promo_mat'><?php $dpt=mysql_query('SELECT * FROM promotion, departement WHERE promotion.id_dpt=departement.id_dpt'); while($array=mysql_fetch_array($dpt)) {echo '<option value='.$array['id_promo'].'>'.$array['nom_dpt'].' '.$array['nom_promo'].' '.$array['annee'].'</option>';} ?></select></div>";
